...
class GetRequestSimulation extends Simulation {
  ...
  val rampUsers=  rampConcurrentUsers(1).to(3).during(10.seconds)
  ...
  val httpScenarios = Map(
    ...
    "all" -> List(
         ...
         GetRequestScenario.getRequestScenario.inject(rampUsers).
                                                                throttle( reachRps(1).in(10.seconds),holdFor(40.seconds),reachRps(1).in(10.seconds)).
...
httpScenarios
  setUp(
    httpScenarios(System.getProperty("Scenario","all")): _*)
